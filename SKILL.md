...
然后读取页面，找到"进入评价"链接并点击进入课程列表。

### Step 2: 获取课程列表

课程列表页面 URL 形如 `xspj_list.do`，包含所有待评价课程。

通过 JavaScript 提取所有未评价课程的链接（URL 中含 `zpf=0` 表示未评价）：
...
(function(){
  ...
  for (var i = 0; i < links.length; i++) {
    if (links[i].textContent.trim() === '评价' && links[i].href.indexOf('zpf=0') >= 0) {
      courses.push(links[i].href);
    }
  }
  return JSON.stringify({total: courses.length, urls: courses});
})()
```
...
执行 `scripts/fill_evaluation.js` 中的脚本。该脚本会：
...
**重要**：填充脚本的参数可以通过修改 `scripts/fill_evaluation.js` 中的变量来调整。详见脚本文件内的注释。
...
所有课程保存后，通过 JavaScript 检查所有课程的评价状态和分数：

```javascript
(function(){
  var links = document.querySelectorAll('a');
  var unevaluated = 0, scores = [];
  for (var i = 0; i < links.length; i++) {
  ...
  return JSON.stringify({unevaluated: unevaluated, scores: scores, min: Math.min.apply(null, scores)});
})()
```
...
* 所有操作通过 `javascript_tool` 执行，减少不必要的页面点击
